...
import { MedicalExamResult } from '@/types/medicalExamination.types';


interface ExamListProps {
  exams: MedicalExamResult[];
  selectedId: string;
  onSelect: (id: string) => void;
}

const ExamList: React.FC<ExamListProps> = ({ exams, selectedId, onSelect }) => (
  <Card>
    <CardHeader>
      ...
      {exams.map((exam) => (
        <div
          ...
          className={`p-3 rounded-lg border cursor-pointer ${
            selectedId === exam.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'
          }`}
